# ARS Codex Pipeline Architecture (tracks ARS v3.12.0 plus mainline updates)

Full pipeline view across stages × workflows × artifacts × gates for the
Codex package. The research logic tracks upstream ARS v3.12.0 plus the
post-tag mainline updates pinned in `manifest.json`, but runtime packaging is different:
Codex installs one router skill at
...
| Cross-model GPT/Gemini dispatch | Disabled in this package; optional external review uses Anthropic Claude Opus 4.8 only when explicitly configured |
...
```mermaid
graph TD
    Pipeline[academic-pipeline<br/>orchestrator<br/>v3.12.0<br/>5 agent prompts]
...
timeline
    ...
    v3.12.0 : post-ship correctness and hardening rollup
